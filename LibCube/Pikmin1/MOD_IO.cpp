...
namespace libcube {
	namespace pikmin1 {
		static void readHeader(oishii::BinaryReader& bReader, Model& mdl)
		{
			skipPadding(bReader);
		...
		static void readCollisionPrism(oishii::BinaryReader& bReader, Model& mdl)
		{
		...
		bool MODImporter::tryRead(oishii::BinaryReader& bReader, pl::FileState& state)
		{
			...
			// Chunk descriptor
			u32 cDescriptor = 0;
			// (u16)-1 means end-of-file for this format
			while ((cDescriptor = bReader.read<u32>()) != (u16) -1)
			{
				...
				case Chunks::CollisionGrid:
					...
					break;
					// Caught because 'EoF' is in every file, therefore we don't need to warn it's unimplemented
				case Chunks::EoF:
					break;
				default:
