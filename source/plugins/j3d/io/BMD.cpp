...
public:
	std::string canRead(const std::string& file, oishii::BinaryReader& reader) const {
		return ends_with(file, "bmd") || ends_with(file, "bdl") ? typeid(Collection).name() : "";
	}
	...
	void read(kpi::IDocumentNode& node, oishii::BinaryReader& reader) const {
		...
		for (const auto& e : ctx.mVertexBufferMaxIndices)
		{
			...
			{
				auto& buf = ctx.mdl.get().mBufs.color[(int)e.first - (int)gx::VertexBufferAttribute::Color0];
			...
			{
				auto& buf = ctx.mdl.get().mBufs.uv[(int)e.first - (int)gx::VertexBufferAttribute::TexCoord0];
				if (buf.mData.size() != e.second + 1)
				{
